...
#' @param .envir an \code{environment} to supply. For normal interactive use, this argument can be ignored.
#' But if this function is to be used programmatically, it is possible that the custom made detectors cannot be found by this function.
#' In this case, simply supply the environment which contains the detectors using this argument.
...
## detect All-------------------------------
detectAll=function(x,plane=NULL,joint=NULL,comp=FALSE,fDict=NULL,.envir=parent.frame()){
  ...
  for(i in 1:length(fnList))detected[[i]]=do.call(fnList[i],list(x),envir=.envir)
